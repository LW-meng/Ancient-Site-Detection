...
test_loader = DataLoader(datasets, batch_size=128, shuffle=False, num_workers=5)
device0 = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

# the following two line should be modified for the change of model architecture
#####################################################################################
model = resnet152(pretrained = False, progress = False)
model.load_state_dict(torch.load('simple/ModelAt74Epoch.pt'))
